...
# time=$(date +"%Y_%m_%d_%H_%M") iwthout am-pm
hostname=$(hostname -s) # -s for short hostname
...
tar -vczf ${dest}/${archive_file} ${backup_files} # &> /dev/null (Bunu script birkaç kez calistirilip eklenebilir)
# tar -xvf ${dest}/${archive_file} kodunu calistirirken root dizinine git cunku extract ederken tekrar backup dizinini olusturmasin. 
